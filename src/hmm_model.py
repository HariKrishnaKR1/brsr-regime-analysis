...
class SustainabilityHMM:
    """HMM for sustainability regime inference."""
    ...
    def _fit_kmeans(self, df: pd.DataFrame, feature_cols: List[str]):
        """Fit K-means for cross-sectional data."""
        ...
        try:
            ...
            self.cluster_to_regime = {np.argsort(intensity)[i]: i for i in range(self.n_states)}
    ...
    def _predict_kmeans(self, df: pd.DataFrame, company_col: str,
        ...
        distances = np.sqrt(np.sum((X - self.cluster_centers_[clusters]) ** 2, axis=1))
        max_distance = np.max(distances) + 1e-8
        regime_probs = 1 - (distances / max_distance)
